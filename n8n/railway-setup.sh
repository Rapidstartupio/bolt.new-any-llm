...
# Load .env if present and RAILWAY_TOKEN not already set
if [ -z "$RAILWAY_TOKEN" ]; then
  if [ -f .env ]; then
    echo "Loading RAILWAY_TOKEN from .env file..."
    set -a
    source .env
    set +a
  fi
...
if ! command -v railway &> /dev/null; then
  echo "Railway CLI not found. Please install with: npm install -g railway"
  exit 1
fi

# Check for Railway token
if [ -z "$RAILWAY_TOKEN" ]; then
  echo "RAILWAY_TOKEN environment variable not set. Please export your Railway API key or add it to .env."
  exit 1
fi
...
# Initialize or select project
railway init --name "$PROJECT_NAME"

# Add Postgres plugin if not already present
if ! railway plugins | grep -q postgresql; then
  railway add --plugin postgresql
fi

# Deploy n8n service (assumes script is run from /n8n)
echo "Deploying n8n service..."
railway up

# Set environment variables for n8n to use internal Postgres
# These will be replaced by Railway with the correct values if Postgres is linked
...
# Optionally set n8n basic auth (customize as needed)
